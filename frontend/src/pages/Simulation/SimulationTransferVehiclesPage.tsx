...
import {
    simListTransferVehicles,
    simAddTransferVehicle,
    simDeleteTransferVehicle,
    type SimulationTransferVehicle,
...
export default function SimulationTransferVehiclesPage() {
    const [veiculos, setVeiculos] = useState<SimulationTransferVehicle[]>([]);
    const [novo, setNovo] = useState<SimulationTransferVehicle>({
        veiculo: "",
        capacidade_kg_min: 0,
        capacidade_kg_max: 0,
        tarifa_km: 0,
    ...
    const carregar = async () => {
        try {
            setLoading(true);
            const data = await simListTransferVehicles();
            setVeiculos(data);
            console.log("Dados recebidos transfer:", data);
        } catch (err) {
    ...
    const salvar = async () => {
        ...
        try {
            setLoading(true);
            await simAddTransferVehicle(novo); // upsert
            await carregar();
    ...
    const iniciarEdicao = (v: SimulationTransferVehicle) => {
        setNovo(v);
    ...
    const excluir = async (veiculo: string) => {
        ...
        try {
            await simDeleteTransferVehicle(veiculo);
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto">
            <h1 className="text-2xl font-semibold mb-6 flex items-center gap-2">
                ...
                Simulação • Veículos Transferência
            </h1>
            ...
            <div className="bg-white rounded-2xl shadow p-4 grid grid-cols-1 md:grid-cols-5 gap-3 mb-6">
                ...
                <div>
                    <label className="block text-sm text-gray-600 mb-1">Tarifa fixa</label>
                    <input
                        type="number"
                        step="0.01"
                        className="input text-right"
                        value={novo.tarifa_fixa}
                        onChange={(e) =>
                            setNovo({ ...novo, tarifa_fixa: Number(e.target.value) })
                        }
                    />
                </div>
                <div>
                    ...
                    <table className="w-full text-sm text-left border-collapse">
                        <thead>
                            <tr className="border-b bg-gray-50">
                                ...
                                <th className="p-2 text-right">Tarifa fixa</th>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filtrados.map((v, i) => (
                                <tr key={i} className="hover:bg-gray-50">
                                    ...
                                    <td className="p-2 text-right">{v.tarifa_fixa.toFixed(2)}</td>
