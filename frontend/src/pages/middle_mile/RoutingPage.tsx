...
export default function RoutingPage() {
    ...
    async function processar() {
        if (!canRun) return alert("Informe a data.");
        try {
            setLoading(true);
            ...
            alert("✅ Roteirização processada.");
        } catch (e) {
            console.error(e);
            alert("❌ Erro ao processar a roteirização.");
        } finally {
            setLoading(false);
        }
    }
    ...
    async function gerarArtefatosEBaixarPDF() {
        ...
        try {
            ...
            const resp = await api.get("/transfer_routing/visualizacao", {
                params: { data_inicial: data }, // sem data_final
                responseType: "blob",
            });
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
                <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Tempo máx. rota (min)</label>
                        <input
                            type="number"
                            min={1}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Parada leve (min)</label>
                        <input
                            type="number"
                            min={0}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Peso leve máx. (kg)</label>
                        <input
                            type="number"
                            min={0}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Parada pesada (min)</label>
                        <input
                            type="number"
                            min={0}
                    ...
                    <div className="md:col-span-2">
                        <label className="block text-sm font-medium text-gray-700 mb-1">Tempo por volume (min/vol.)</label>
                        <input
                            type="number"
                            step="0.01"
                            min={0}
                    ...
                    <div className="flex items-center gap-2 pt-6">
                        ...
                        />
                        <label htmlFor="modo_forcar" className="text-sm text-gray-700">Forçar sobrescrita</label>
                    </div>
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                    ...
                    <button
                        ...
                        disabled={!canRun || loading}
                        className="bg-sky-600 hover:bg-sky-700 text-white px-4 py-2 rounded-lg disabled:opacity-60"
                    >
                        {loading ? "Gerando..." : "Gerar & Baixar PDF"}
                    </button>
                </div>
            </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
                    <div className="flex flex-wrap items-center gap-3">
                        {artefatos.map_html_url && (
                            <a href={artefatos.map_html_url} target="_blank" rel="noreferrer" className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg" download>
                                Baixar Mapa HTML
                            </a>
                        )}
                        {artefatos.pdf_url && (
                            <a href={artefatos.pdf_url} target="_blank" rel="noreferrer" className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg" download>
                                Baixar PDF
                            </a>
                        )}
                    </div>
                    ...
                    {artefatos.map_html_url ? (
                        <div className="mt-4 border rounded-lg overflow-hidden">
                            <iframe key={iframeKey} src={artefatos.map_html_url} title="Mapa Interativo - Transferências" className="w-full" style={{ height: "70vh" }} />
                        </div>
                    ) : (
                        <p className="mt-4 text-sm text-gray-500">Nenhum mapa disponível para a data selecionada.</p>
                    )}
                </div>
            )}
        </div>
