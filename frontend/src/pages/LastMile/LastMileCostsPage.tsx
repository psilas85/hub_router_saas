...
export default function LastMileCostsPage() {
    ...
    const processar = async () => {
        ...
        setLoading(true);
        try {
            // 🔑 mesma data como inicial e final
            const r = await lmProcessCosts(data, data);
            setMsg(r?.mensagem ?? "Cálculo de custos processado.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const visualizar = async () => {
        if (!dataVisualizar) return alert("Selecione a data para visualizar relatórios.");
        setLoading(true);
        try {
            const r = await lmVisualizeCosts(dataVisualizar);
            setArtefatos(r);
            if (!r?.json_dados || r.json_dados.length === 0) {
                setMsg("⚠️ Nenhum dado encontrado para esta data.");
            } else {
                setMsg("✅ Artefatos carregados.");
            }
        } catch (err) {
            console.error(err);
            setMsg("❌ Erro ao buscar artefatos.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto">
            <h1 className="text-2xl font-semibold mb-6">Last-Mile • Custeio</h1>
            ...
            <div className="bg-white rounded-2xl shadow p-4 grid grid-cols-1 md:grid-cols-5 gap-3 mb-4">
                ...
                <div className="flex items-end gap-2 md:col-span-2">
                    <button className="btn-secondary" onClick={visualizar} disabled={loading}>
                        {loading ? "Buscando…" : "Gerar Detalhes/Resumo/PDF"}
                    </button>
                </div>
            </div>
            ...
            {artefatos && artefatos.json_dados && artefatos.json_dados.length > 0 && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mt-4">
                    <h2 className="font-semibold mb-3">Artefatos {artefatos.envio_data}</h2>
                        ...
                        <table className="min-w-full border border-gray-200 divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-3 py-2 text-right text-sm font-medium text-gray-600 border-b">Entregas</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {artefatos.json_dados.map((row: any, i: number) => (
                                    <tr key={i} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-3 py-2 border-b text-right">{row.quantidade_entregas}</td>
                                        <td className="px-3 py-2 border-b text-right">{row.peso_total_kg?.toLocaleString("pt-BR")}</td>
                                        <td className="px-3 py-2 border-b text-right">{row.distancia_total_km?.toLocaleString("pt-BR")}</td>
                                        <td className="px-3 py-2 border-b text-right">
                                            {row.custo_entrega_total?.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                                        </td>
                                        <td className="px-3 py-2 border-b text-right">
                                            {row.cte_frete_total?.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                                        </td>
                                        <td className="px-3 py-2 border-b text-right">
                                            {row.percentual_custo ? row.percentual_custo.toFixed(2) + "%" : "-"}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                            <tfoot className="bg-gray-100 font-semibold">
                                {(() => {
                                    const totalEntregas = artefatos.json_dados.reduce((acc, r) => acc + (r.quantidade_entregas || 0), 0);
                                    const totalPeso = artefatos.json_dados.reduce((acc, r) => acc + (r.peso_total_kg || 0), 0);
                                    const totalDist = artefatos.json_dados.reduce((acc, r) => acc + (r.distancia_total_km || 0), 0);
                                    const totalCusto = artefatos.json_dados.reduce((acc, r) => acc + (r.custo_entrega_total || 0), 0);
                                    const totalFrete = artefatos.json_dados.reduce((acc, r) => acc + (r.cte_frete_total || 0), 0);
                                    const percentualTotal = totalFrete > 0 ? (totalCusto / totalFrete) * 100 : 0;

                                    return (
                                        <tr>
                                            <td className="px-3 py-2 border-t text-right">Totais:</td>
                                            <td className="px-3 py-2 border-t text-right">{totalEntregas}</td>
                                            <td className="px-3 py-2 border-t text-right">{totalPeso.toLocaleString("pt-BR")}</td>
                                            <td className="px-3 py-2 border-t text-right">{totalDist.toLocaleString("pt-BR")}</td>
