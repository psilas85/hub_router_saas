// src/pages/Data_input/DataInputPage.tsx
import { useState } from "react";
import api from "@/services/api";

type Resultado = {
    status: string;
    tenant_id: string;
    mensagem: string;
    total_processados?: number;
    validos?: number;
    invalidos?: number;
};

export default function DataInputPage() {
    const [file, setFile] = useState<File | null>(null);
    const [modoForcar, setModoForcar] = useState(false);
    const [limitePeso, setLimitePeso] = useState<number | "">("");
    const [loading, setLoading] = useState(false);
    const [erro, setErro] = useState<string | null>(null);
    const [resultado, setResultado] = useState<Resultado | null>(null);

    const limparEstado = () => {
        setFile(null);
        setModoForcar(false);
        setLimitePeso("");
        setErro(null);
        // mantém o resultado para o usuário ver
    };

    const processar = async () => {
        setErro(null);
        setResultado(null);

        if (!file) {
            setErro("Selecione um arquivo .csv primeiro.");
            return;
        }

        try {
            setLoading(true);

            const formData = new FormData();
            formData.append("file", file);

            const params = new URLSearchParams();
            params.append("modo_forcar", String(modoForcar));
            if (limitePeso !== "") params.append("limite_peso_kg", String(limitePeso));

            const { data } = await api.post<Resultado>(
                `/data_input/upload?${params.toString()}`,
                formData,
                { headers: { "Content-Type": "multipart/form-data" } }
            );

            setResultado(data);
            // limpa apenas os campos de entrada
            setFile(null);
            setModoForcar(false);
            // mantém limitePeso se quiser repetir com mesmo parâmetro
        } catch (err: any) {
            const detail = err?.response?.data?.detail || err.message || "Erro desconhecido";
            setErro(typeof detail === "string" ? detail : JSON.stringify(detail));
        } finally {
            setLoading(false);
        }
    };

    return (
        <div className="max-w-3xl mx-auto p-6">
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
                📂 Data Input
            </h2>

            {/* Card de upload */}
            <div className="bg-white rounded-lg shadow p-5 space-y-4">
                <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium text-gray-700">Arquivo (.csv)</label>
                    <input
                        type="file"
                        accept=".csv"
                        onChange={(e) => setFile(e.target.files?.[0] || null)}
                        className="block w-full text-sm text-gray-600
              file:mr-4 file:py-2 file:px-4
              file:rounded-md file:border-0
              file:text-sm file:font-semibold
              file:bg-emerald-600 file:text-white
              hover:file:bg-emerald-700"
                    />
                    {file && (
                        <span className="text-xs text-gray-500">
                            Selecionado: <strong>{file.name}</strong> ({Math.ceil(file.size / 1024)} KB)
                        </span>
                    )}
                </div>

                <label className="flex items-center gap-2 text-sm">
                    <input
                        type="checkbox"
                        checked={modoForcar}
                        onChange={(e) => setModoForcar(e.target.checked)}
                        className="h-4 w-4 text-emerald-600 border-gray-300 rounded"
                    />
                    Forçar reprocessamento
                </label>

                <div className="flex flex-col gap-1">
                    <label className="text-sm font-medium text-gray-700">
                        Limite de peso (kg) <span className="text-gray-400 font-normal">(opcional, padrão 15000)</span>
                    </label>
                    <input
                        type="number"
                        placeholder="15000"
                        value={limitePeso}
                        onChange={(e) => setLimitePeso(e.target.value === "" ? "" : Number(e.target.value))}
                        className="w-full border rounded px-3 py-2 text-sm"
                    />
                </div>

                <div className="flex gap-2">
                    <button
                        disabled={!file || loading}
                        onClick={processar}
                        className="bg-emerald-600 text-white px-4 py-2 rounded hover:bg-emerald-700 disabled:opacity-50"
                    >
                        {loading ? "⏳ Processando..." : "Processar Data Input"}
                    </button>
                    <button
                        type="button"
                        onClick={limparEstado}
                        className="px-4 py-2 rounded border text-gray-700 hover:bg-gray-50"
                        disabled={loading}
                    >
                        Limpar
                    </button>
                </div>

                {erro && (
                    <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded p-3">
                        ❌ {erro}
                    </div>
                )}
            </div>

            {/* Card de resultado */}
            {resultado && (
                <div className="mt-6 bg-white rounded-lg shadow p-5">
                    <h3 className="font-semibold mb-3">Resultado</h3>
                    <p className="text-sm text-gray-700 mb-3">{resultado.mensagem}</p>

                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                        <div className="border rounded p-3">
                            <div className="text-gray-500">Total processados</div>
                            <div className="text-lg font-semibold">
                                {resultado.total_processados ?? "—"}
                            </div>
                        </div>
                        <div className="border rounded p-3">
                            <div className="text-gray-500">Válidos</div>
                            <div className="text-lg font-semibold text-emerald-700">
                                {resultado.validos ?? "—"}
                            </div>
                        </div>
                        <div className="border rounded p-3">
                            <div className="text-gray-500">Inválidos</div>
                            <div className="text-lg font-semibold text-rose-700">
                                {resultado.invalidos ?? "—"}
                            </div>
                        </div>
                    </div>
                </div>
            )}
        </div>
    );
}
